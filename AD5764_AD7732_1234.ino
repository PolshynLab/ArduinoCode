...
void bufferRamp(std::vector<String> DB)
{
  ...
  for (int j=0; j<nSteps;j++)
  {
    ...
    for(int i = 0; i < NchannelsDAC; i++)
    {
      ...
      writeDAC_buffer(4-(channelsDAC[i]-'0'),v);
    }
    digitalWrite(ldac,LOW);
    digitalWrite(ldac,HIGH);
    if (delayUnit)
    ...
    {
      rampRead(channelsADC[i]-'1', b1, b2, &b1, &b2, count,DB[NchannelsDAC*2+5].toInt());
      count+=1;
    }
    if(Serial.available())
    {
...
void autoRamp2(std::vector<String> DB)
{
  ...
  byte b1;
  byte b2;
  int dacChannel1=4-DB[1].toInt();
  int dacChannel2=4-DB[2].toInt();
...
void router(std::vector<String> DB)
{
  ...
  switch ( operation )
  {
    ...
    case 2:
    switch_channel = 4-DB[1].toInt();     // Switch the channel
    ...
    v = writeDAC(switch_channel,DB[2].toFloat());
    Serial.print("DAC ");
    Serial.print(DB[1]);
